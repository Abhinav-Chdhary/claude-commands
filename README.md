...
| [`/ac:greptileit`](plugins/ac/commands/greptileit.md) | Opens a PR, drives it to a clean Greptile review (checking the score in the PR description, not just the comments, and catching a stale one), fixes real findings for up to 5 rounds, then merges and cleans up the branch and worktree. Stops for any product decision. |
...
All commands land under the `/ac:` prefix — `/ac:standup`, `/ac:reviewinator`, `/ac:endit`, `/ac:scratchpad`, `/ac:greptileit`.
...
/ac:greptileit              # PR the current branch, review-loop it, merge, clean up
/ac:greptileit add rate limiting to the ingest endpoint
```
